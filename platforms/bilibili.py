...
class BilibiliPlatform(MusicPlatform):
    ...
    def __init__(self):
        ...
        # cid 由 BV 号唯一确定且固定不变，播放时缓存避免重复请求 view 接口
        self._cid_cache: dict[str, str] = {}

    async def _get_wbi_key(self) -> str:
        # wbi key 有效期约 1 天，超过 6 小时重新拉取，避免长期运行后签名失效
        if self._wbi_key and time.time() - self._wbi_key_ts < 6 * 3600:
    ...
    async def _get_cid(self, bvid: str) -> str:
        cached = self._cid_cache.get(bvid)
        if cached:
            return cached
        async with self._client() as client:
            resp = await client.get(
    ...
    async def get_download_url(self, song: Song, stream_type: str = "video") -> str:
        ...
        # video 档：与在线播放一致，返回含音频的完整 MP4 流
        # （DASH 视频轨为纯视频无音轨，不能作为有声下载源）
